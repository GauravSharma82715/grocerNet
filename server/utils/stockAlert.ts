...
/**
 * Checks remaining inventory for a product and dispatches an email alert
 * to store administrators if stock has fallen below the threshold.
 * Asynchronous and fully isolated with try/catch to never block caller requests.
 */
export const checkAndAlertLowStock = async (productId: string): Promise<void> => {
    try {
                    ...
                    <div style="padding: 28px;">
                        <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px;">
                            ...
                            <div>
                                ...
                                <p style="margin: 0; font-size: 14px; color: #6b7280;">${product.category} • ${product.unit || "unit"}</p>
                            </div>
                        </div>
        ...
        console.error("Failed to check or send low-stock alert:", error);
    }
};
