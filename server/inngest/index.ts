...
export const inngest = new Inngest({ id: "my-grocerNet" });

// Low stock alert event to admin
const checkLowStock = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await step.run("send low-stock-alert-emails", async () => {
...
const sendMonthlyOffers = inngest.createFunction(
    {
        ...
        if (users.length === 0 || deals.length === 0) {
            return { skipped: true, reason: "No data found" }
        }
        let sentCount = 0;
        ...
        for (let i = 0; i < users.length; i += batchSize) {
            ...
            await step.run(`send-batch-${i}`, async () => {
                for (const u of batch) {
                    await sendEmail({
                        to: u.email,
                        subject: "GrocerNet: Monthly Deals just dropped 💰",
                        body: ` <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        //skip if order doesn't exist or already has a rider
        ...
        history.push({
            status: "Assigned",
            note: `Automatically assigned to ${availableRider.name}`,
            timestamp: new Date(),
