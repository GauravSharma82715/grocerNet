...
const Products = () => {
  ...
  const updateFilter = (key: string, value: string) => {
    ...
    // Close mobile filter panel after selecting a filter
    if (key === "category" || key === "organic" || key === "sort") {
          ...
          <main className="flex-1">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="lg:hidden flex items-center justify-center gap-2 flex-1 px-3 py-2 text-sm bg-white rounded-xl border border-app-border hover:bg-app-cream transition-colors"
              >
              ...
              <div className="relative flex-1 lg:flex-none">
                <select
                  ...
                  className="appearance-none w-full lg:w-auto pl-3 pr-8 py-2 text-sm bg-white rounded-xl border border-app-border focus:border-app-green outline-none cursor-pointer"
                >
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-sm text-app-text-light mb-4">
                  {" "}
                  Tru adjusting your filters or search terms
                </p>
                <button
                  onClick={clearFilter}
                  className="px-5 py-2 text-sm font-medium bg-app-green text-white rounded-xlhover:bg-app-green-light transition-colors"
                >
                  Clear Filters
                </button>
              </div>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4xl:gap-8">
                {products.map(
                  (product) =>
                    product.stock > 0 && (
                ...
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    className={`size-9 rounded-lg text-sm font-medium transition-colors ${page === i + 1
                      ...
                      : "bg-white text-app-light hover:bg-app-cream"
                      }`}
                  >
                    {i + 1}
                  </button>
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white z-50 rounded-t-2xl max-h-[80vh] overflow-y-auto animate-slide-in-up">
            <div className="flex items-center justify-between p-4 border-b border-app-border">
              ...
              <button
                onClick={() => setMobileFiltersOpen(false)}
                className=" p2 hover:bg-app-cream rounded-lg"
              >
                <X className="size-5" />
              </button>
            </div>
